...
```ts
import type { TestimonialRecord, TestimonialSchemaTypes } from "@/schemas/testimonialSchema";
...
const { data: approved } = await testimonialService.getAllSorted<TestimonialRecord>({
